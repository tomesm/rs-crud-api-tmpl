(function () {
    ...
    ***************************************************************************** */
    ...
    async function execute(httpMethod, path, data) {
    ...
    // export as singleton
    const passengerMco = new PassengerMco();

    var _PassengercMvc_passengerInputEl, _PassengercMvc_passengerListEl, _PassengerInput_inputEl, _PassengerItem_titleEl, _PassengerItem_data;
    let PassengercMvc = class PassengercMvc extends BaseHTMLElement {
        constructor() {
            super(...arguments);
            _PassengercMvc_passengerInputEl.set(this, void 0);
            _PassengercMvc_passengerListEl.set(this, void 0);
        ...
        `;
            _a = this, _b = this, [({ set value(_c) { __classPrivateFieldSet(_a, _PassengercMvc_passengerInputEl, _c, "f"); } }).value, ({ set value(_c) { __classPrivateFieldSet(_b, _PassengercMvc_passengerListEl, _c, "f"); } }).value] = scanChild(htmlContent, "passenger-input", "passenger-list");
            this.append(htmlContent);
            this.refresh();
        }
        async refresh() {
            let passengers = await passengerMco.list();
            let htmlContent = document.createDocumentFragment();
            for (const passenger of passengers) {
                const el = document.createElement("passenger-item");
                el.data = passenger; // passenger will be frozen
                htmlContent.append(el);
            }
            __classPrivateFieldGet(this, _PassengercMvc_passengerListEl, "f").innerHTML = "";
            __classPrivateFieldGet(this, _PassengercMvc_passengerListEl, "f").append(htmlContent);
        }
        // #region    --- UI Events
        onCheckPassenger(evt) {
            const passengerItem = evt.selectTarget.closest("todo-item");
        ...
        // #endregion --- UI Events
        // #region    --- Data Events
        onPassengerUpdate(data) {
            // find the todo in the UI
            const passengerItem = first(`passenger-item.Passenger-${data.id}`);
            // if found, update it.
            if (passengerItem) {
                passengerItem.data = data; // data will be frozen
            }
        }
        onPassengerCreate(data) {
            this.refresh();
        }
    };
    _PassengercMvc_passengerInputEl = new WeakMap();
    _PassengercMvc_passengerListEl = new WeakMap();
    __decorate([
        onEvent('pointerup', 'c-check')
    ], PassengercMvc.prototype, "onCheckPassenger", null);
    __decorate([
        onHub('dataHub', 'Passenger', 'update')
    ], PassengercMvc.prototype, "onPassengerUpdate", null);
    __decorate([
        onHub('dataHub', 'Passenger', 'create')
    ], PassengercMvc.prototype, "onPassengerCreate", null);
    PassengercMvc = __decorate([
        customElement("passenger-mvc")
    ], PassengercMvc);
    let PassengerInput = class PassengerInput extends BaseHTMLElement {
        constructor() {
            super(...arguments);
            _PassengerInput_inputEl.set(this, void 0);
            // #endregion --- UI Events
        }
        init() {
            let htmlContent = html `
        ...
        `;
            __classPrivateFieldSet(this, _PassengerInput_inputEl, scanChild(htmlContent, "input"), "f");
            this.append(htmlContent);
        }
        // #region    --- UI Events
        onInputKeyUp(evt) {
            if (evt.key == "Enter") {
                // get value from UI
                const first_name = __classPrivateFieldGet(this, _PassengerInput_inputEl, "f").value;
                // send create to server
                passengerMco.create({ first_name });
                // don't wait, reset value input
                __classPrivateFieldGet(this, _PassengerInput_inputEl, "f").value = '';
            }
        }
    };
    _PassengerInput_inputEl = new WeakMap();
    __decorate([
        onEvent('keyup', 'input')
    ], PassengerInput.prototype, "onInputKeyUp", null);
    PassengerInput = __decorate([
        customElement("passenger-input")
    ], PassengerInput);
    let PassengerItem = class PassengerItem extends BaseHTMLElement {
        constructor() {
            super(...arguments);
            _PassengerItem_titleEl.set(this, void 0);
            _PassengerItem_data.set(this, void 0);
        }
        set data(data) {
            let oldData = __classPrivateFieldGet(this, _PassengerItem_data, "f");
            __classPrivateFieldSet(this, _PassengerItem_data, Object.freeze(data), "f"); // make date immutable for now
            if (this.isConnected) {
                this.refresh(oldData);
            }
        }
        get data() { return __classPrivateFieldGet(this, _PassengerItem_data, "f"); }
        init() {
            let htmlContent = html `
        ...
        `;
            __classPrivateFieldSet(this, _PassengerItem_titleEl, scanChild(htmlContent, "div"), "f");
            this.append(htmlContent);
            this.refresh();
        }
        refresh(old) {
            ...
            //render new data
            const passenger = __classPrivateFieldGet(this, _PassengerItem_data, "f");
            this.classList.add(`Passenger-${passenger.id}`);
            this.classList.add(passenger.status);
            __classPrivateFieldGet(this, _PassengerItem_titleEl, "f").textContent = `${passenger.first_name} ${passenger.last_name}`;
        }
    };
    _PassengerItem_titleEl = new WeakMap();
    _PassengerItem_data = new WeakMap();
    PassengerItem = __decorate([
